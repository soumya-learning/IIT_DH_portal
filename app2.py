...
enrollments_df = fetch_table("enrollments")   # ← NEW
...
# ─────────────────────────────────────────────────────────────────────────────
# Enrollment helper — detect column names once
# ─────────────────────────────────────────────────────────────────────────────
enroll_stu_col  = None
enroll_code_col = None
if not enrollments_df.empty:
    enroll_stu_col  = next((c for c in ["student_id", "roll_no", "roll"] if c in enrollments_df.columns), None)
    enroll_code_col = next((c for c in ["course_code", "course_id", "code"] if c in enrollments_df.columns), None)




def get_enrolled_students(course_code):
    """Return a filtered students_df containing only students enrolled in course_code."""
    if enrollments_df.empty or not enroll_stu_col or not enroll_code_col or not stu_id_col_s:
        return students_df  # fallback: show all students if no enrollment table
    enrolled_ids = enrollments_df[
        enrollments_df[enroll_code_col].astype(str) == str(course_code)
    ][enroll_stu_col].tolist()
    return students_df[students_df[stu_id_col_s].isin(enrolled_ids)]
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if courses_df.empty or attendance_df.empty or students_df.empty:
        ...
        for _, course_row in filtered_courses.iterrows():
            ...
            # ── KEY CHANGE: only enrolled students for this course ────────────
            enrolled_students_df = get_enrolled_students(code)
            ...
            if stu_id_col_a and stu_id_col_s:
                ...
                # Filter rows to matching students when searching by student
                ...
                orig_cols  = [c for c in enrolled_students_df.columns if c not in HIDDEN_COLS]
            ...
            expander_label = (
                ...
                f"   |   Enrolled: {len(enrolled_students_df)} student(s)"
            )
            ...
            with st.expander(expander_label, expanded=bool(search_query)):
                ...
                ci4.markdown(f"**Enrolled:** {len(enrolled_students_df)} student(s)")
                ...
                if display_df.empty:
                    st.info("No students match your search for this course." if search_query else "No students enrolled in this course.")
                    continue
